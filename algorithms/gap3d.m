function theta_3d = gap3d(y,A_fun,At_fun,Row,Col,T,block,spbasis,m_star,stopc,acc,weight_base,weighttype)
%GAP3D  Generalized alternating projection for three-dimensional compressive
%sensing problems.
...
% Phi_sum = sum(A_fun(At_fun(eye(size(y,1)))),2); % [cs] replaced by function handle
...
for iter_i = 1:(iternum)
    ...
    if(acc)
        ...
        ytemp = y0_2d - A_fun(reshape(theta_3d,[row*col,T])); % [cs] replaced by function handle
        y_2d = y0_2d + ytemp;
    else
        y_2d = y0_2d;
    end
    ...
    y_temp = (y_2d-A_fun(reshape(theta_3d,[row*col,T])))./(Phi_sum); % [cs] replaced by function handle
    % y_temp = (y_2d-A_fun(reshape(theta_3d,[row*col,T]))); % [cs] replaced by function handle
    ...
    lambda = sum_sort(m_star+1);  % m_star should be decreased for 2D-CS
    ...
    L2_err = norm(y0_2d - A_fun(reshape(theta_3d,[row*col,T]))); % [cs] replaced by function handle
